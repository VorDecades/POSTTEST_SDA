...
void tampilkanStokSembako(BarangNode* head) {
    ...
    if (head == nullptr) {  // Cek apakah list kosong, jika memang kosong, output Gudang kosong
        cout << "Gudang kosong." << endl;
        return;  // Keluar dari fungsi
    }

    BarangNode* temp = head;  // Pointer dimulai dari head

    do {
        // Cetak nama barang dan stok node saat ini
        cout << "- " << temp->namaBarang << ": " << temp->stok << endl;
        temp = temp->next;  // Pindah ke node berikutnya
    } while (temp != head);  // Berhenti saat temp kembali ke head (sudah satu putaran)
